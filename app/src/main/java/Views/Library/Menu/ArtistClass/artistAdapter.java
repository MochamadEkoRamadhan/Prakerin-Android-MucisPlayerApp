...
import ekoramadhan.shape.Library.allsong.itemMenu;
import ekoramadhan.shape.Library.allsong.itemRect;
import ekoramadhan.shape.Library.allsong.itemStick;
...
public class artistAdapter extends BaseAdapter {
    ...
    public artistAdapter(){
        String[] albums_cols = { MediaStore.Audio.Artists.ARTIST,MediaStore.Audio.Artists._ID, MediaStore.Audio.Artists.NUMBER_OF_TRACKS};
        Cursor cursor = Ui.ef.getBaseContext().getContentResolver().query( MediaStore.Audio.Artists.EXTERNAL_CONTENT_URI,albums_cols,null,null, MediaStore.Audio.Artists.ARTIST +" COLLATE NOCASE ASC ");
        ...
        for (int i = 0; i < cursor.getCount(); i++) {
            ...
            if(!str.equals("") && !index.containsKey(str)){
                 ...
                 data.add(new String[]{cursor.getString(0),cursor.getString(1),cursor.getString(2),null});
            }
        }
           ...
           /* ShapeView stick = itemStick.getFMview(context,false);
            ...
            menu.onClick(new call(){
                ...
                public void onCall(boolean bl) {
                    ...
                    final artitstBtns mn = new artitstBtns(getContext(), Ui.cd.DPW, Ui.cd.DPH,data.get(position));
                    //mn.fromHtml = true;
