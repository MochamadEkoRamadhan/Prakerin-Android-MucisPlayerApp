...
import ekoramadhan.player.playlistHandler;
import ekoramadhan.shape.Library.Icon.addNextIcon;
import ekoramadhan.shape.Library.Icon.fileInfoIcon;
import ekoramadhan.shape.Library.Icon.folderIcon;
import ekoramadhan.shape.Library.Icon.genreIcon;
import ekoramadhan.shape.Library.Icon.playAllIcon;
import ekoramadhan.shape.Library.Icon.ringToneIcon;
import ekoramadhan.shape.Library.Icon.shareFileIcon;
import ekoramadhan.shape.Library.allsong.menucloseBtn;
import ekoramadhan.shape.home.menu.backgroundImg;
import ekoramadhan.shape.home.menu.btmBack;
...
public class genersBtns extends FMlyt {
    ...
    public genersBtns(Context context, int width, int height, String[] data) {
        ...
        for(int i = 0;i < len;i++){
            for(int j = 0;j < line;j++){
                if((j*len) + i < 6){
                    ...
                    switch ((j*len) + i){
                        case 0:
                            item.setData("PLAY", new playAllIcon(iconSize,iconSize,0,0));
                            break;
                        case 1:
            ...
            Ui.bk.add(new call(){
                ...
                public void onCall(boolean bl) {
                    ...
                    if(fromHtml){
                        ...
                        final songsPopup mn = new songsPopup(getContext(), Ui.cd.DPW, Ui.cd.DPH,genersBtns.this.data);
